...
The layout is built lazily, the first time the control is both visible and attached to a handler, rather than in the constructor. A picker that is never shown — a collapsed field in a dynamic form, an unselected tab — therefore allocates no child views at all, and one that is shown builds exactly once, after its initial property values have landed. Setting `IsVisible` to `True` later builds it on the spot.

The calendar section is a three-row `Grid`: a header row with prev/next buttons (plus an optional Today button) and a tappable month/year label, a fixed day-names row, and a container that holds both the day-cell grid and the month/year picker grid (only one is visible at a time).

The day-cell grid is a 6×7 `Grid` whose 42 cells are created once and then re-stamped in place — nothing after the first build allocates a view. Every cell has the same shape: a container `Grid` holding a centred `Border` around the day `Label`, so each state is expressed purely as a change of colour. Today is a 2 px stroke `Ellipse`; selected dates and range endpoints are a filled `Ellipse`; a plain day is the same `Border` with a transparent background and zero stroke thickness. Dates between the range start and end use `RangeHighlightColor` as their container background to produce a continuous band. Disabled dates are gated when the cell is tapped rather than by adding and removing the gesture recognizer.

Because of that, the appearance properties — `TodayHighlightColor`, `SelectionColor`, `RangeHighlightColor`, `HeaderBackgroundColor`, `DayNameColor`, `DayColor`, `DisabledDayColor`, `OtherMonthDayColor` — repaint the existing cells rather than rebuilding them, which matters when they are bound with `DynamicResource` for light/dark theming. `MinimumDate`, `MaximumDate` and the range properties repaint too, since none of them change the grid's shape or the dates it shows. Only month navigation re-assigns dates to the cells.
